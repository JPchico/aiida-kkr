...
class KkrCalculation(CalcJob):
    ...
    # mandatory if noncollinear directions are used that are not (theta, phi)= (0,0) for all atoms
    _NONCO_ANGLES = 'nonco_angle.dat'
    # mandatory for GREENIMP option (scattering code)
    _NONCO_ANGLES_IMP = 'nonco_angle_imp.dat'
    # mandatory for GREENIMP option (scattering code)
    _SHAPEFUN_IMP = 'shapefun_imp'
    # mandatory for GREENIMP option (scattering code)
    _POTENTIAL_IMP = 'potential_imp'
    ...
    _ALL_KKRFLEX_FILES = [_KKRFLEX_GREEN, _KKRFLEX_TMAT,
                          _KKRFLEX_ATOMINFO, _KKRFLEX_INTERCELL_REF, _KKRFLEX_INTERCELL_CMOMS]
    # Jij files
    ...
    def define(cls, spec):
        ...
        spec.input('parent_folder', valid_type=RemoteData, required=True,
                   help='Use a remote or local repository folder as parent folder (also for restarts and similar). It should contain all the  needed files for a KKR calc, only edited files should be uploaded from the repository.')
        spec.input('impurity_info', valid_type=Dict, required=False,
                   help='Use a Parameter node that specifies properties for a follwoing impurity calculation (e.g. setting of impurity cluster in scoef file that is automatically created).')
        spec.input('kpoints', valid_type=KpointsData, required=False,
                   help="Use a KpointsData node that specifies the kpoints for which a bandstructure (i.e. 'qdos') calculation should be performed.")
...
""")
    ...
    def prepare_for_submission(self, tempfolder):
        ...
        try:
            ...
            self.logger.error(
                "Failed trying to find input parameter of parent {}".format(parent_calc))
                    ...
                    try:
                        ...
                        if value == oldvalue:
                            ...
                            if type(value) == float:
                                ...
                                tmp_value, tmp_oldvalue = np.array(
                                    value).reshape(-1), np.array(oldvalue).reshape(-1)
                        ...
                        raise InputValidationError(
                            "Error while trying to compare old and new values with key={} in do_never_modify list, oldval={}; newval={}".format(key, oldvalue, value))
                    if not values_eqivalent:
                        self.logger.error(
                            "You are trying to set keyword {} = {} but this is not allowed since the structure would be modified. Please use a suitable workfunction instead.".format(key, value))
                        raise InputValidationError(
                            "You are trying to modify a keyword that is not allowed to be changed! (key={}, oldvalue={}, newvalue={})".format(key, oldvalue, value))
        ...
        try:
            ...
            self.logger.error(
                'KkrCalculation: Could not get structure from Voronoi parent ({}).'.format(parent_calc))
            raise ValidationError(
                "Cound not find structure node from parent {}".format(parent_calc))
        ...
        if kkrflex_opt:
            ...
            parameters = update_params_wf(parameters, Dict(
                dict={'RUNOPT': runopt, 'nodename': 'update_KKRFLEX', 'nodedesc': 'Update Parameter node with KKRFLEX runopt'}))
        ...
        if found_imp_info and write_scoef:
            ...
            if 'imp_cls' not in imp_info_dict:
                ...
                cylinder_orient = imp_info_dict.get(
                    'cylinder_orient', [0., 0., 1.])
                ...
                if ilayer_center > len(structure.sites) - 1:
                    raise IndexError('Index of the reference site is out of range! Possible values: 0 to {}.'.format(
                        len(structure.sites) - 1))
                elif Rcut < 0:
                    raise ValueError('Cutoff radius has to be positive!')
                elif not vec_shape or len(cylinder_orient) != 3:
                    raise TypeError(
                        'Input orientation vector ({}) has the wrong shape! It needs to be a 3D-vector!'.format(cylinder_orient))
                ...
                print('Write scoef from imp_cls input!', len(
                    imp_info.get_dict().get('imp_cls')))
                with tempfolder.open(self._SCOEF, 'w') as scoef_file:
                    if alat_input is not None:
                        alat = get_alat_from_bravais(
                            np.array(structure.cell), structure.pbc[2])
                        rescale_alat = alat/alat_input
                        self.report(
                            "INFO: rescaling imp cls due to alat_input: {}".format(rescale_alat))
                    else:
                        rescale_alat = None
                    write_scoef_full_imp_cls(
                        imp_info, scoef_file, rescale_alat)
        ...
        if found_kpath:
            parameters = self._prepare_qdos_calc(
                parameters, kpath, structure, tempfolder, use_alat_input)
        ...
        if 'initial_noco_angles' in self.inputs:
            parameters = self._use_initial_noco_angles(
                parameters, structure, tempfolder)
        ...
        with tempfolder.open(self._INPUT_FILE_NAME, u'w') as input_file:
            natom, nspin, newsosol, warnings_write_inputcard = generate_inputcard_from_structure(
                parameters, structure, input_file, parent_calc, shapes=shapes, vca_structure=vca_structure, use_input_alat=use_alat_input)
        ...
        if has_parent:
            ...
            for file1 in copylist:
                ...
                if self._SHAPEFUN not in copylist:
                    ...
                    local_copy_list.append(
                        (voro_retrieved.uuid, VoronoiCalculation._SHAPEFUN, self._SHAPEFUN))

            # check if core state lie within energy contour and take them out if needed
            local_copy_list = self._kick_out_corestates_kkrhost(
                local_copy_list, tempfolder)
            ...
            if ef_set is not None:
                local_copy_list = self._set_ef_value_potential(
                    ef_set, local_copy_list, tempfolder)
        ...
        if retrieve_dos_files:
            print(
                'adding files for dos output',
                self._COMPLEXDOS, self._DOS_ATOM, self._LMDOS
            )
            ...
            for iatom in range(natom):
                ...
                for ispin in range(nspin):
                    add_files.append(
                        (self._LMDOS % (iatom+1, ispin+1)).replace(' ', '0'))
        ...
        if retrieve_qdos_files:
            ...
            for iatom in range(natom):
                for ispin in range(nspin):
                    add_files.append((self._QDOS_ATOM %
                                      (iatom+1, ispin+1)).replace(' ', '0'))
                    # try to retrieve both old and new version of the files
                    add_files.append((self._QDOS_ATOM_OLD %
                                      (iatom+1, ispin+1)).replace(' ', '0'))
        ...
        if retrieve_Jij_files:
            add_files = [self._SHELLS_DAT] + [self._Jij_ATOM %
                                              iatom for iatom in range(1, natom+1)]
    ...
    def _set_parent_remotedata(self, remotedata):
        ...
        if input_remote:
            raise ValidationError(
                "Cannot set several parent calculation to a KKR calculation")
    ...
    def _set_ef_value_potential(self, ef_set, local_copy_list, tempfolder):
        ...
        if not has_potfile or len(txt) == 0:
            ...
            potcopy_info = [i for i in local_copy_list if i[2]
                            == self._POTENTIAL][0]
        ...
        for ipotstart in potstart:
            self.report(
                f"set ef {ef_set} in potential starting in line {ipotstart}")
    ...
    def _prepare_qdos_calc(self, parameters, kpath, structure, tempfolder, use_alat_input):
        ...
        alat = get_alat_from_bravais(
            np.array(structure.cell), is3D=structure.pbc[2]) * get_Ang2aBohr()
        if use_alat_input:
            alat_input = parameters.get_dict().get('ALATBASIS')
        else:
            alat_input = alat
        kpath_array = kpath_array * \
            (alat_input/alat) / get_Ang2aBohr() / (2*np.pi/alat)
        # now write file
        qvec = ['%i\n' % len(kpath_array)]
        qvec += ['%e %e %e\n' % (kpt[0], kpt[1], kpt[2])
    ...
    def _use_initial_noco_angles(self, parameters, structure, tempfolder):
        ...
        """
        self.report(
            'Found `initial_noco_angles` input node, writing nonco_angle.dat file')
        ...
        with tempfolder.open(self._NONCO_ANGLES, 'w') as noco_angle_file:
            for iatom in range(natom):
                ...
                # write line
                noco_angle_file.write(
                    f'   {theta}    {phi}    {fix_dir[iatom]}\n')
    ...
    def _use_decimation(self, parameters, tempfolder):
        ...
        """
        self.report(
            'Found `deciout_parent` input node, activae decimation mode')
        ...
        if self._DECIFILE not in deciout_retrieved.list_object_names():
            raise InputValidationError(
                "Error: deciout_parent does not contain decifile!")
        ...
        change_values.append(['RUNOPT', runopt])
        # needed to make DECIFILE work
        change_values.append(['FILES', [self._POTENTIAL, self._SHAPEFUN]])
        # works only for right continuation for now!
        change_values.append(['DECIFILES', ['vacuum', self._DECIFILE]])
        # decimation works only in one-shot mode
        change_values.append(['NSTEPS', 1])
        parameters = _update_params(parameters, change_values)
