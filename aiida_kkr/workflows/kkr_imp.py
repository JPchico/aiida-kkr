...
class kkr_imp_wc(WorkChain):
    ...
    def get_wf_defaults(cls, silent=False):
        ...
        return cls._options_default, cls._wf_default, cls._voro_aux_default
    ...
    def define(cls, spec):
        ...
        spec.input(
            ...
            required=True,
            help="""
            KKRhost code used to run GF writeout step.
            """,
        )
        spec.input(
            'voronoi',
        ...
        spec.input(
            ...
            required=True,
            help="""
            KKRimp code used to converge the impurity calculation
            """,
        )
        spec.input(
            'impurity_info',
            ...
            required=True,
            help="""
            Information of the impurity like position in the unit cell,
            screening cluster, atom type.
            """,
        )
        spec.input(
            'remote_data_host',
            ...
            required=False,
            help="""
            RemoteData node of the converged host calculation.
            Used to write out the host Green function.
            """,
        )
        spec.input(
            'remote_data_gf',
        ...
        spec.input(
            ...
            required=False,
            help="""
            RemoteData node of precomputed host Green function with Fermi level
            shift (overwrite kkrflex_green and tmat files from first
            remote_data_gf node.
        """,
        )
        spec.input(
        ...
        spec.input(
            ...
            required=False,
            help="""
            Options for running the Voronoi code
            (if differing from general `options` node)
            """,
        )
        spec.input(
            'voro_aux_parameters',
            ...
            required=False,
            help="""
            Parameters for the auxiliary voronoi starting potential workflow.
            """,
        )
        spec.input(
            'wf_parameters',
        ...
        spec.input(
            ...
            required=False,
            help="""
            If given, overwrite the some parameters used as input for auxiliary
            voronoi calculation of starting potential.
            """,
        )
        spec.input(
            ...
            required=False,
            help="""
            Set some input parameters of the KKR calculation for the GF
            writeout step.
            """,
        )
        spec.input(
            ...
            required=False,
            help="""
            Set starting potential (e.g. from preconverged calculation
            """,
        )
        ...
        spec.outline(
            # initialize workflow
            cls.start,
            # validate the input (if true, run_gf_writeout, else skip)
            if_(cls.validate_input)(
                # write out the host GF
                cls.run_gf_writeout
            ),
            # check if strarting potential exists in input already
            # (otherwise create it)
            if_(cls.has_starting_potential_input)(
                # calculate the auxiliary impurity potentials
                cls.run_voroaux,
                # construct the host-impurity startpotential
                cls.construct_startpot
            ),
            # run the kkrimp_sub workflow to converge the host-imp startpot
            cls.run_kkrimp_scf,
            # check if the calculation was successful and return the result
            # nodes
            cls.return_results,
        )
    ...
    def start(self):
        ...
        self.ctx.description_wf = self.inputs.get(
            'description',
            'Workflow for a KKR impurity calculation starting from a host-impurity potential',
        )
        self.ctx.label_wf = self.inputs.get('label', 'kkr_imp_sub_wc')

        # set parameters for the auxiliary voronoi calculation
        self.ctx.voro_dos_params = voro_aux_dict.get(
            'dos_params',
            self._voro_aux_default['dos_params'],
        )
        self.ctx.voro_num_rerun = voro_aux_dict.get(
            'num_rerun',
            self._voro_aux_default['num_rerun'],
        )
        ...
        self.ctx.voro_natom_in_cls_min = voro_aux_dict.get(
            'natom_in_cls_min',
            self._voro_aux_default['natom_in_cls_min'],
        )
        self.ctx.voro_delta_e_min = voro_aux_dict.get(
            'delta_e_min',
            self._voro_aux_default['delta_e_min'],
        )
        self.ctx.voro_threshold_dos_zero = voro_aux_dict.get(
            'threshold_dos_zero',
            self._voro_aux_default['threshold_dos_zero'],
        )
        self.ctx.voro_check_dos = voro_aux_dict.get(
            'check_dos',
            self._voro_aux_default['check_dos'],
        )
        self.ctx.voro_delta_e_min_core_states = voro_aux_dict.get(
            'delta_e_min_core_states',
            self._voro_aux_default['delta_e_min_core_states'],
        )
        # set up new parameter dict to pass to voronoi subworkflow later
        self.ctx.voro_params_dict = orm.Dict(
            dict={
                'queue_name': self.ctx.queue,
                'resources': self.ctx.resources,
                'max_wallclock_seconds': self.ctx.max_wallclock_seconds,
        ...
        self.ctx.kkr_runmax = wf_dict.get(
            'kkr_runmax',
        ...
        self.ctx.aggressive_mix = wf_dict.get(
            'aggressive_mix',
        ...
        self.ctx.broyden_number = wf_dict.get(
            'broyden-number',
        ...
        self.ctx.init_pos = wf_dict.get(
            'init_pos',
            self._wf_default['init_pos'],
        )
        self.ctx.accuracy_params = wf_dict.get(
            'accuracy_params',
        ...
        self.report(
            ...
            f'parameters for the voroaux calculation: {self.ctx.voro_params_dict.get_dict()}\n'
            f'parameters for the kkrimp scf: {self.ctx.kkrimp_params_dict.get_dict()}\n'
        )
    ...
    def validate_input(self):
        ...
        if 'kkrimp' in inputs and 'voronoi' in inputs:
            try:
        ...
        if 'remote_data_gf' in inputs and 'remote_data_host' in inputs:
            self.report(
                f'INFO: both converged host remote (pid: {inputs.remote_data_host.pk})'
                f' and GF writeout remote (pid: {inputs.remote_data_gf.pk}) found in input. '
                'Converged host remote will not be used. Skip GF writeout step and '
                'start workflow with auxiliary voronoi calculations.'
            )
            do_gf_calc = False
        elif 'remote_data_host' in inputs:
            self.report(
                f'INFO: found converged host remote (pid: {inputs.remote_data_host.pk}) in input. '
                'Start workflow by calculating the host GF.'
            )
            if 'kkr' in inputs:
                do_gf_calc = True
            else:
            ...
            remote_data_gf_node = orm.load_node(inputs.remote_data_gf.pk)
            pk_kkrflex_writeoutcalc = remote_data_gf_node.get_incoming(link_label_filter=u'remote_folder'
                                                                       ).first().node.pk
            self.report(
                f'INFO: found remote_data node (pid: {inputs.remote_data_gf.pk})'
                f' from previous KKRFLEX calculation (pid: {pk_kkrflex_writeoutcalc}) in input. '
            ...
            if 'remote_data_gf_Efshift' in inputs:
                self.report(
                    f'INFO: found remote_data_gf_Efshift'
                    f' (pid: {inputs.remote_data_gf_Efshift.pk}) used to overwrite Fermi level.'
                )
        else:
            inputs_ok = False
    ...
    def run_gf_writeout(self):
        """Run the gf_writeout workflow to calculate the host Green's function.

        It writes the KKR flexfiles using the converged host remote folder
        and the impurity info node.
        """
        ...
        sub_description = f'GF writeout sub workflow for kkrimp_wc using'\
            + f' converged host remote data (pid: {converged_host_remote.pk})'\
            + f' and impurity_info node (pid: {imp_info.pk})'
    ...
    def run_voroaux(self):
        ...
        if self.ctx.do_gf_calc:
            ...
            remote_data_gf_node = orm.load_node(self.inputs.remote_data_gf.pk)
        ...
        # set Fermi level for auxiliary impurity potential correctly
        # (extract from EF that is used in impurity calc)
        ...
        updatenode = orm.Dict(dict={'ef_set': set_efermi, 'add_direct': True})
        ...
        # add some voronoi specific parameters automatically if found
        # (RMTREF should also set RMTCORE to the same value)
        ...
        for key, val in self.ctx.change_voro_params.items():
            ...
            changed_params = True
        if changed_params:
            updatenode = orm.Dict(dict=calc_params_dict)
        ...
        sub_description += f'{imp_info.get_dict().get("Zimp")}'\
            + f' in the host structure from pid: {converged_host_remote.pk}'
        ...
        builder.options = orm.Dict(dict=self.ctx.options_params_dict_voronoi)
        future = self.submit(builder)

        tmp_calcname = 'voro_aux_1'
    ...
    def get_ef_from_parent(self):
        ...
        if self.ctx.do_gf_calc:
            ...
            parent_remote = orm.load_node(self.inputs.remote_data_gf_Efshift.pk)
        else:
            parent_remote = orm.load_node(self.inputs.remote_data_gf.pk)
    ...
    def construct_startpot(self):
        ...
        if self.ctx.do_gf_calc:
            ...
            GF_host_calc = orm.load_node(GF_host_calc_pk)
            converged_host_remote = self.inputs.remote_data_host
        else:
            remote_data_gf_node = orm.load_node(self.inputs.remote_data_gf.pk)
            GF_host_calc = remote_data_gf_node.get_incoming(link_label_filter=u'remote_folder').first().node
        ...
        all_nodes = self.ctx.last_voro_calc.get_outgoing(node_class=orm.CalcJobNode).all()
        print(all_nodes)
        pk_last_voronoi = max([i.node.pk for i in all_nodes])
        print(pk_last_voronoi)
        voro_calc_remote = orm.load_node(pk_last_voronoi).outputs.remote_folder
        print(voro_calc_remote)

        print(orm.load_node(pk_last_voronoi).outputs.retrieved.list_object_names())
        print(GF_host_calc)
        ...
        caching_info = f'INFO: cache_source of voronoi node: {orm.load_node(pk_last_voronoi).get_cache_source()}'
        print(caching_info)
        self.report(caching_info)
        ...
        settings_label = f'startpot_KKRimp for imp_info node {imp_info}'
        ...
        startpot_kkrimp = neworder_potential_wf(
            ...
            parent_calc_folder=converged_host_remote,
        ...
        self.report(
            f'INFO: created startpotential (pid: {startpot_kkrimp}) for the impurity calculation '
            f'by using information of the GF host calculation (pid: {GF_host_calc.pk}), the potential of the '
            f'converged host system (remote pid: {converged_host_remote.pk}) and the potential of the auxiliary voronoi '
            f'calculation (remote pid: {self.ctx.last_voro_calc.pk})'
        )
    ...
    def run_kkrimp_scf(self):
        ...
        sub_description = f'convergence of the host-impurity potential (pk: {startpot.pk}) using GF remote (pk: {gf_remote.pk})'
    ...
    def return_results(self):
        ...
        if self.ctx.kkrimp_scf_sub.is_finished_ok:
            ...
            last_calc_output_params = orm.load_node(last_calc_pk).outputs.output_parameters
            ...
            outputnode_dict['last_calculation_uuid'] = orm.load_node(last_calc_pk).uuid
    ...
    def final_cleanup(self):
        ...
        if self.ctx.create_startpot:
            ...
            for fname in ret.list_object_names():
                if fname not in [
                    VoronoiCalculation._OUTPUT_FILE_NAME,
...
def change_struc_imp_aux_wf(struc, imp_info):  # Note: works for single imp at center only!
    ...
    new_struc = orm.StructureData(cell=struc.cell)
